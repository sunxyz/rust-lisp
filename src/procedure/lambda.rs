...
fn lambda(apply_args: &mut ApplyArgs, lazy_eval: bool) -> Box<dyn Fn(&mut ApplyArgs) -> LispType + Send + Sync> {
                ...
                if (name == ".") {
                    if (args_name.len() == 2) {
                        ...
                        if let Symbol(name) = key {
                            env.try_write().expect("locked err").define(name.as_str(), Expr(args_val.clone()));
                    ...
                    // println!("{}:{}", name, v);
                    env.try_write().expect("locked err").define(name.as_str(), v.clone());
                }
            }
            _ => {
