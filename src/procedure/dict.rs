...
fn dict_rm(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==2{
        if let Dict( dict) = args.car(){
            ...
            dict.write().expect("error").remove(&key.to_string());
            Nil
        }else{
...
fn dict_get(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==2{
        if let Dict( dict) = args.car(){
            ...
            if let Some(val) = dict.read().expect("error").get(&key){
                val.clone()
            }else{
                Nil
            }
...
fn dict_put(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==3{
        if let Dict( dict) = args.car(){
            ...
            dict.write().expect("error").insert(key, val.clone());
            Nil
        }else{
...
fn dict_contains_key(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==2{
        if let Dict( dict) = args.car(){
            ...
            LispType::Boolean(dict.read().expect("error").contains_key(&key))
        }else{
...
fn dict_len(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==1{
        if let Dict( dict) = args.car(){
            LispType::Number(dict.read().expect("error").len() as isize)
        }else{
...
fn dict_keys2list(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==1{
        if let Dict( dict) = args.car(){
            ...
            for key in dict.read().expect("error").keys(){
...
fn dict_value2list(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==1{
        if let Dict( dict) = args.car(){
            let mut list = List::new();
            for val in dict.read().expect("error").values(){
                list.push(val.clone());
            }
            Expr(list)
        }else{
...
fn dict2list(apply_args: &mut ApplyArgs) -> LispType {
    ...
    if args.len()==1{
        if let Dict( dict) = args.car(){
            let mut list = List::new();
            for (key,val) in dict.read().expect("error").iter(){
